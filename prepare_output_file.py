...
location
dinner_dates
number of dinners between 4/1/2016 - 10/1/16
number of dinners between 10/2/16 - 3/31/2017
number of dinners between 7/1/2016 - 10/1/16
number of dinners between 10/2/16 - 12/31/2016
...
def read_dataset(filename, cols, as_dict=False):
    content = open(str(Path(filename).resolve()), 'rb').read().decode('utf-8')
    ds = Dataset().load(content, format='csv')
    remove = set(ds.headers) - set(cols)
    for col in remove:
        del ds[col]
    return dict(list(ds)) if as_dict else ds
...
def main():
    ...
    for p in profiles.dict:
        ...
        ds.append(r)
        print(r)
    open('output.csv', 'wb').write(ds.csv.encode('utf-8'))
    print('Done.')
